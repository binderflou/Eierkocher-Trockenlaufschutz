...
| **R2.2** | Kontinuierlicher Soll/Ist-Vergleich, Sicherheitsmodus | ControlLogic | `SystemController`, `SafetyManager` | `executeCycle()` `checkDryRun()` | – | `executeCycle()` `checkDryRun()` | I1, I3 (bestanden) |
| **R2.3** | Fehleranalyse: Sensorfehler, Überhitzung | ControlLogic | `SafetyManager`, `StateDetector` | `checkDryRun()` `handleError()` | – | `checkDryRun()` | – |
...
| **R3.2** | Warnungen: Anzeigewechsel, akustisches Signal | UserInterface | `DisplayController`, `BuzzerController` | `updateDisplay()` `playErrorTone()` | – | `updateDisplay()` `playErrorTone()` (rudimentär) | I2 (bestanden)|
...
| **R4.2** | Fehlerklassifizierung | ControlLogic / UI | `SafetyManager`, `DisplayController` | `checkDryRun()` `showWarning()` | 1 | Nicht implementiert | – |
