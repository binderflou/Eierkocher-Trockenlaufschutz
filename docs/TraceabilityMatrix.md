...
| **R2.2** | Kontinuierlicher Soll/Ist-Vergleich, Sicherheitsmodus | ControlLogic | `SystemController`, `SafetyManager` | `executeCycle()` `checkDryRun()` | 2 | `executeCycle()` `checkDryRun()` | I1, I3 (bestanden) |
| **R2.3** | Fehleranalyse: Sensorfehler, Überhitzung | ControlLogic | `SafetyManager`, `StateDetector` | `checkDryRun()` `handleError()` | 2 | `checkDryRun()` | – |
...
| **R4.1** | Plausibilitätsprüfung beim Einschalten | ControlLogic / HardwareAbstraction | `SystemController`, `FillLevelSensor`, `TemperatureSensor` | `validate()` `readLevel()`, `readTemperature()` | 2 | Teilweise | – |
| **R4.2** | Fehlerklassifizierung | ControlLogic / UI | `SafetyManager`, `DisplayController` | `checkDryRun()` `showWarning()` | 2 | Nicht implementiert | – |
| **R4.3** | Zyklischer Selbsttest (30s) | ControlLogic | `SystemController`, `SafetyManager` | `executeCycle()` `handleError()` | 2 | Nicht implementiert | – |
...
| **R5.3** | Warnanzeige (Farbschema, Symbole) | UserInterface | `DisplayController` | `updateDisplay(status)` | – | `updateDisplay(status)` (Teilweise) | I2 (bestanden) |
| **R5.4** | Fehlermeldung Trockenlauf, Quittierung | UI / ControlLogic | `DisplayController`, `SystemController` | `showWarning()` `acknowledgeError()` | – | `showWarning()` | – |
| **R5.5** | Akustisches Warnsignal | UserInterface | `BuzzerController` | `playWarningTone()` `playErrorTone()` | – | `playWarningTone()` `playErrorTone()` (Teilweise) | – |
